...
void Mesh::initialize_decimation_callbacks() {
  ...
  custom_pre_collapse_callback =
    ...
    // The circulation functino only returns faces around one vertex of the edge
    // (circulates the edge ccw or cw) Thus, we get both and then insert them
    // into a set to get a unique list of faces
    V2Fe.clear();
  ...
  cost_and_placement_callback = [&](const int e, const MatrixXd &V,
                                    const MatrixXi &F, const MatrixXi &E,
                                    const VectorXi &EMAP, const MatrixXi &EF,
                                    const MatrixXi &EI, double &cost,
                                    RowVectorXd &p) {
    // cost = frame_field_alignment_data(e, E); -> another cost function based
    // on only frame-field alignment
  ...
  custom_post_collapse_callback =
      [&](const MatrixXd &V, const MatrixXi &F, const MatrixXi &E,
          ...
          const int e2, const int f1, const int f2,
          const bool collapsed) -> bool {
    if (!collapsed)
      return false;

    // update the frame field by averaging the frame field of the two vertices
    // that are collapsed
    ...
    // Note, we are updating both as it seems there's no convention for which we
    // know which vertex is the one kept
    ...
    // PD2.row(v1)
    std::vector<VectorXd> Dir = {PD1.row(v1), PD2.row(v1), -PD1.row(v1),
                                 -PD2.row(v1)};
    ...
    for (int f : V2Fe) {
      ...
      for (int i = 0; i < 3; i++) {
        int ei =
            EMAP(i * F.rows() + f); // Need to Make sure this indexing is right
    ...
    /*std::cout << "direction 1: " << max_alignment[0].first << ", "
      << max_alignment[0].second.first << ", " << max_alignment[0].second.second
    << std::endl; std::cout << "direction 2: " << max_alignment[1].first << ", "
      << max_alignment[1].second.first << ", " << max_alignment[1].second.second
    << std::endl; std::cout << "direction 3: " << max_alignment[2].first << ", "
      << max_alignment[2].second.first << ", " << max_alignment[2].second.second
    << std::endl; std::cout << "direction 4: " << max_alignment[3].first << ", "
      << max_alignment[3].second.first << ", " << max_alignment[3].second.second
    << std::endl;*/
   ...
   Matrix<int, Dynamic, 3> Fn(F);
    ...
    // now we have the best alignment for each direction, we can flip the edges
    // accordingly
    /// The code below crashes after some iterations (index out of range), not
    /// yet sure why tho. (TODO: Hossam)
    // for (int j = 0; j < 4; j++)
    //{
...
void Mesh::decimate() {
  ...
  std::cout << "Face and Vert Size: " << F.rows() << " " << V.rows()
            << std::endl;
  std::cout << "PD PV Size: " << PD1.rows() << " " << PD2.rows() << " "
            << PV1.rows() << " " << PV2.rows() << std::endl;
  ...
  igl::max_faces_stopping_condition(curr_nF, orig_nF, target_nF,
                                    stopping_condition_callback);

  VectorXi J, I;
  igl::decimate(V, F, cost_and_placement_callback, stopping_condition_callback,
                custom_pre_collapse_callback, custom_post_collapse_callback, V,
                F, J, I);
}
...
void Mesh::compute_before(const int e, const MatrixXd &V, const MatrixXi &E,
                          const MatrixXi &F, const VectorXi &EMAP,
                          const MatrixXi &EF, const MatrixXi &EI,
                          int &valenceSum, double &alignment,
  ...
  for (int i = 0; i < valenceSum; i++) {
    ...
    alignmentTotal += alignment_function(PD1.row(Nv[i]), PD2.row(Nv[i]),
                                         V.row(Nv[i]) - V.row(E.col(0)(e)));
  }
...
void Mesh::compute_after(
  ...
  for (int i = 0; i < neighbors.size(); i++) {
    alignmentTotal += alignment_function(
        PD1.row(neighbors[i]), PD2.row(neighbors[i]), V.row(neighbors[i]) - p);
  }
  alignment = alignmentTotal / neighbors.size();
  valenceSum = neighbors.size() == 6 ? 1 : 0;
}

/*// The objective is to find the vertex position p that minimizes the quadratic
error function p^T Q p,
  ...
  )
{
    ...
    // The new position is obtained from the first three elements of the result
vector RowVector3d newPosition(result[0], result[1], result[2]); return
newPosition;
}*/

MatrixXd Mesh::compute_quadric(const int e, const MatrixXd &V,
                               const MatrixXi &E, const MatrixXi &F,
                               const VectorXi &EMAP, const MatrixXi &EF,
                               const MatrixXi &EI) {
  MatrixXd quadric;
  quadric.setZero(4, 4);
...
void Mesh::qcoarsen_based_cost(const int e, const MatrixXd &V,
                               const MatrixXi &E, const MatrixXi &F,
                               const VectorXi &EMAP, const MatrixXi &EF,
                               const MatrixXi &EI, const RowVector3d &p,
                               double &cost, MatrixXd quadricErrorTerm) {
  ...
  double vB_A = (V.rows() + 1) * total_ideal /
                (V.rows() * (total_ideal + idealCountA - idealCountB));
  double dist = (V.row(E(e, 0)) - V.row(E(e, 1))).norm();
  double weight = 1; // placeholder
  cost = std::pow((vB_A + alpha), 2) * std::pow((alignB / alignA + alpha), 2) *
         std::pow((dist * weight + alpha), 2);
