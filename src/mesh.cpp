...
void Mesh::initialize_decimation_callbacks()
{
                                    ...
                                    Eigen::RowVectorXd &p)
  {
    ...
    qcoarsen_based_cost(e, V, E, F, EMAP, EF, EI, PD1, PD2, PV1, PV2, p, cost); 
...
void Mesh::compute_before(const int e,
	...
	const Eigen::MatrixXi & EF,
	const Eigen::MatrixXi & EI,
	const Eigen::MatrixXd & PD1,//frame field 1s
	const Eigen::MatrixXd & PD2,//frame field 2s
	int & valenceSum,
  	double & alignment,
  ...
  double & alignment)
{
		...
		for (int i = 0; i < neighbors.size(); i++) {
			alignmentTotal += alignment_function(v.row(neighbors[i]), w.row(neighbors[i]),V.row(neighbors[i])-p);
		}
	...
	)
{
  ...
  std::vector<int> neighbors;
  compute_before(e,V,E,F,EMAP,EF,EI,PD1,PD2,idealCountB,alignB,neighbors);
  compute_after(V, neighbors, p, PD1, PD2, idealCountA, alignA);
  double alpha = 0.001;
