...
void per_vertex_point_to_plane_quadrics_aligned(
  ...
  quadrics.resize(V.rows(),
                  ...
                  Quadric{Eigen::MatrixXd::Zero(dim, dim),
                          Eigen::RowVectorXd::Zero(dim), 0});
  ...
  for (int f = 0; f < F.rows(); f++) {
    ...
    for (int c = 0; c < 3; c++) {
      if (std::isinf(V(F(f, c), 0)) || std::isinf(V(F(f, c), 1)) ||
          std::isinf(V(F(f, c), 2))) {
    ...
    const auto subspace_quadric = [&I](const Eigen::RowVectorXd &p,
      ...
      for (int i = 0; i < m; i++) {
        ...
        for (int j = 0; j < i; j++)
          assert(std::abs(S.row(j).dot(ei)) < 1e-10);
        A += -ei.transpose() * ei;
        b += p.dot(ei) * ei;
        c += -pow(p.dot(ei), 2);
      }
    ...
    if (infinite_corner == -1) {
      ...
      int n = EF(e, opp);
      int nc = EI(e, opp);
      assert(((F(f, (infinite_corner + 1) % 3) == F(n, (nc + 1) % 3) &&
      ...
      for (int c = 0; c < 3; c++) {
        if (c != infinite_corner) {
          quadrics[F(f, c)] =
              igl::operator+(quadrics[F(f, c)], boundary_edge_quadric);
        }
      }
    }
  }
