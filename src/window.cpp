...
void Window::visualizeFrameFields(RowVector3d maxCurvatureColor,
  ...
  viewer.data().add_edges(
      mesh.V.array() +
          (mesh.PD1.array() * mesh.PV1.replicate(1, 3).array() * factor),
      mesh.V.array() -
          (mesh.PD1.array() * mesh.PV1.replicate(1, 3).array() * factor),
      maxCurvatureColor);
  ...
  viewer.data().add_edges(
      mesh.V.array() +
          (mesh.PD2.array() * mesh.PV2.replicate(1, 3).array() * factor),
      mesh.V.array() -
          (mesh.PD2.array() * mesh.PV2.replicate(1, 3).array() * factor),
      minCurvatureColor);
}
...
void Window::visualizeMetricOnEdges() {
  ...
  MatrixXi E;
  igl::edges(mesh.F, E);
  // std::cout << mesh.F.rows() << " " << E.rows() << " " << E.cols() <<
  // std::endl;
...
void Window::initialize_callbacks() {
  const auto &key_down = [&](igl::opengl::glfw::Viewer &viewer,
                             unsigned char key, int mod) -> bool {
    switch (key) {
    case 'C':
    case 'c':
      visualizeFrameFields(RowVector3d(0.8, 0.2, 0.2),
                           RowVector3d(0.2, 0.2, 0.8));
      break;
    case 'V':
    case 'v':
      mesh.decimate();
